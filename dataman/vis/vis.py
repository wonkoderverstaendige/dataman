...
class Vis(app.Canvas):
    def __init__(self, target):
        ...
        self.buf = Buffer()
        self.buf.initialize(nChannels=self.n_channels, nSamples=self.n_samples, nptype='float32')
        print self.buf.nChannels, self.buf.nSamples
        self.buf.put_data(np.zeros((self.n_channels, self.n_samples), dtype=np.float32))
        ...
        self.program['a_position'] = self.buf.get_data(0, self.n_samples).reshape(-1, 1)
    ...
    def on_timer(self, event):
        ...
        for i in range(self.n_channels):
            data = read_record(os.path.join(self.target, '106_CH{}.continuous'.format(i+1)), offset=self.offset)[:self.n_samples]
            self.buf.put_data(data[:], start=0, channel=i)
        self.program['a_position'].set_data(self.buf.get_data(0, self.n_samples))
